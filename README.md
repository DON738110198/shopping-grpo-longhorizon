...
下表是上游发布的历史结果，不是当前 385/43 数据重新训练后的结果。三个模型在
相同的 200 道留出任务上各进行一次确定性 Rollout：
...
### 上游 SFT LoRA 历史训练（448 条训练数据，3 个 epoch）
...
| 环节 | 耗时估算 |
|---|---:|
| Teacher 采集（616 条，4 workers） | ~68 分钟 |
| 200 任务评测（Base） | ~20 分钟 |
